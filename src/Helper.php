...
if (!class_exists(__NAMESPACE__ . '\Helper')) {
    class Helper
    {
        ...
        public static function time_elapsed_string($datetime, $full = false)
        {
            ...
            // Determine if the given date is in the past or future
            $isFuture = $now < $ago;
        ...
        public static function ngrok($param = 'public_html', $port = 4040, $tunnel = 0)
        {
            ...
            return (isset($j) &&
                    ...
                    isset($j['tunnels']) &&
                    is_array($j['tunnels']) &&
                    isset($j['tunnels'][$tunnel]) &&
                    is_array($j['tunnels'][0]) &&
                    isset($j['tunnels'][$tunnel][$param]))
                ? $j['tunnels'][$tunnel][$param]
                : null;
        }
            ...
            return (
                (
                    (date('Y-m-d', strtotime($t->validatedat)) >= '2020-09-18' &&
                        date('Y-m-d', strtotime($t->validatedat)) <= '2020-10-01') or
                    (date('Y-m-d', strtotime($t->validatedat)) >= '2020-11-7' &&
